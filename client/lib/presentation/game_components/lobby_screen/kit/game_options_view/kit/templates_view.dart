...
class TemplatesView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final getTemplateSelector = (BoxConstraints constraints) => GameOptionView(
        ...
        dropdownItemWidth: constraints.maxWidth - 250,
        dropdownOptions: [
          defaultValue,
          ...templates.map((temp) => temp.name).toList()
        ],
        dropdownDefaultValueIdx: defaultIndex != -1 ? defaultIndex + 1 : null,
        onDropdownOptionChangedOrOptionToggled: (val) {
          if (val != null) {
            final template = templates.any((temp) => temp.name == val)
                ? templates.firstWhere((temp) => temp.name == val)
                : null;
            lobbyCubit.setGameTemplate(template?.id);
          }
        });
